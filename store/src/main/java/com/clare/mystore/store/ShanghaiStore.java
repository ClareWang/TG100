...
public class ShanghaiStore extends Store implements Runnable
{
	
	private Thread cook=new Thread(this);
	
	public ShanghaiStore() 
	{
		this.name="ShanghaiStore";
		this.num=0;
	...
	private void addBaoziIntoList(Cargo cargo) 
	{
		for(int i=0;i<cargo.getBaoziList().size();i++)
		{
			...
			{
			case DOUSHA:
				baozi=new DoushaBaozi(Taste.ORGINAL);
				break;
			case TANG:
				baozi=new TangBaozi(Taste.ORGINAL);
				break;
			case NIUROU:
				baozi=new NiurouBaozi(Taste.ORGINAL);
				break;
			case ZHUROU:
				baozi=new ZhurouBaozi(Taste.ORGINAL);
				break;
			default:
